...
class IEL:
    ...
    def get_nn_energy(self,params,mm_energy):
        ...
        "checks the di-nucleotides in invader and sequence"
        nn_seq = []

        for i in range(self.length - 1):
            nn_key = f"{self.seq[i]}{self.seq[i + 1]}/{self.invader[i]}{self.invader[i + 1]}"
            energy = nn_params.get(nn_key, -1.5)
            nn_seq.append((nn_key, energy))
            self.alterations_energy[i + 1] += energy
            self.alterations[i + 1] = (
